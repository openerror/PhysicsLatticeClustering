...
def extract_cluster_coordinates(
    HCStats: NDArray, sim_params_dict: dict[str, int]
) -> Iterable[tuple[NDArray, NDArray, bool, int]]:
    """
    ...
    X = HCStats[:, 0]
    Y = HCStats[:, 1]  # X,Y coordinates of ALL identified clusters
...
def find_cluster_grouping(cluster_coordinates, sim_params_dict):
    ...
    points_at_boundary = [
        _find_boundary_points(pts, sim_params_dict)
        for pts in boundary_touching_clusters
    ]
...
def _detect_touch(X, Y, sim_params_dict, touch_threshold=1):
    ...
    assert len(X) == len(
        Y
    ), "WARNINGS: lengths of the provided coordinate arrays must match!"
